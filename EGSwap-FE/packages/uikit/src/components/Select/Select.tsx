...
const DropDownHeader = styled.div`
  ...
  border: 1px solid ${({ theme }) => theme.colors.pool.inputSecondary};
  border-radius: 6px;
  background: ${({ theme }) => theme.colors.pool.input};
...
const DropDownContainer = styled(Box)<{ isOpen: boolean }>`
  ...
  ${(props) =>
    ...
    css`
      ${DropDownHeader} {
        border-bottom: 1px solid ${({ theme }) => theme.colors.pool.inputSecondary};
...
const ListItem = styled.li`
  ...
  padding: 8px 16px;
  &:hover {
    background: ${({ theme }) => theme.colors.pool.inputSecondary};
  }
`;
  ...
  return (
    <DropDownContainer isOpen={isOpen} {...props}>
      <DropDownHeader onClick={toggling}>
        <Text color={pool ? "#495B8D" : !optionSelected && placeHolderText ? "text" : undefined}>
