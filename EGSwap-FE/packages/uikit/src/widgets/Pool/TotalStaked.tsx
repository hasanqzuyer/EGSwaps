...
export const TotalToken = ({
  ...
  if (total && total.gte(0)) {
    return (
      <Balance fontSize="16px" value={getBalanceNumber(total, tokenDecimals)} decimals={decimalsToShow} unit={` ${symbol}`} fontWeight="700" />
    );
  }
...
> = ({ totalStaked, tokenDecimals, decimalsToShow, symbol }) => {
  ...
  return (
    <StatWrapper
      label={
        <TooltipText ref={targetRef} small color="pool.textSubtle">
          {t("Total staked")}:
        </TooltipText>
      }
    >
