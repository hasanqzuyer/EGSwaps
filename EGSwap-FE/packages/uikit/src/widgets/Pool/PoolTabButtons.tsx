...
const PoolTabButtons = ({
  ...
  const liveOrFinishedSwitch = (
    <Wrapper>
      <ButtonMenu activeIndex={isExact ? 0 : 1} scale="sm" variant="subtle" style={{gap: 4}}>
        <ButtonMenuItem as={NextLinkFromReactRouter} to="/pools" pool replace style={{}}>
          {t("Live")}
        </ButtonMenuItem>
        <NotificationDot show={hasStakeInFinishedPools}>
          <ButtonMenuItem id="finished-pools-button" as={NextLinkFromReactRouter} to="/pools/history" replace pool style={{borderRadius: 6, color: '#fff', border: !theme.isDark? '1px  solid #009BCC' : 'none'}}>
            {t("Finished")}
          </ButtonMenuItem>
        </NotificationDot>
      </ButtonMenu>
