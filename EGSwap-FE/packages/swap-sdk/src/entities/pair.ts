...
import { FACTORY_ADDRESS_MAP,  INIT_CODE_HASH_MAP,  PANCAKESWAP_FACTORY_ADDRESS_MAP, PANCKAESWAP_INIT_CODE_HASH_MAP } from '../constants'
...
export const computePairAddress = ({
  ...
  tokenA: ERC20Token
  tokenB: ERC20Token
}, externalSwap: boolean = false): string => {
  const [token0, token1] = tokenA.sortsBefore(tokenB) ? [tokenA, tokenB] : [tokenB, tokenA] // does safety checks

  if(externalSwap){
    return getCreate2Address(
      PANCAKESWAP_FACTORY_ADDRESS_MAP[token0.chainId],
      keccak256(['bytes'], [pack(['address', 'address'], [token0.address, token1.address])]),
      PANCKAESWAP_INIT_CODE_HASH_MAP[token0.chainId]
    )
  }
...
export class Pair {
  ...
  public static getAddress(tokenA: ERC20Token, tokenB: ERC20Token, externalSwap: boolean = false): string {
    return computePairAddress({ factoryAddress: FACTORY_ADDRESS_MAP[tokenA.chainId], tokenA, tokenB }, externalSwap)
  }

  public constructor(currencyAmountA: CurrencyAmount<ERC20Token>, tokenAmountB: CurrencyAmount<ERC20Token>, externalSwap: boolean = false) {
    const tokenAmounts = currencyAmountA.currency.sortsBefore(tokenAmountB.currency) // does safety checks
      ? [currencyAmountA, tokenAmountB]
      : [tokenAmountB, currencyAmountA]
    this.liquidityToken = new ERC20Token(
      tokenAmounts[0].currency.chainId,
      Pair.getAddress(tokenAmounts[0].currency, tokenAmounts[1].currency, externalSwap),
      18,
      'EGSwap-LP',
      'EGSwap LP'
    )
