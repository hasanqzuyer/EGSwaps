        ...
        <RowBetween key={token.address} width="100%">
          <RowFixed>
            ...
            <a href={getBlockExploreLink(token.address, 'token', chainId)} target="_blank" rel="noreferrer noopener">
              <BscScanIcon width="20px" color="newPrimary" />
            </a>
          </RowFixed>
          <RowFixed>
            <IconButton variant="text" onClick={() => removeToken(chainId, token.address)}>
              <DeleteOutlineIcon color="newPrimary" />
            </IconButton>
          </RowFixed>
        </RowBetween>
      ))
  ...
  return (
    <Wrapper>
      <Column style={{ width: '100%', flex: '1 1' }}>
        ...
        {tokenList}
        <Footer>
          <Text bold color={isBlue ? "#9A6AFF" : "#22CE77"}>
            {userAddedTokens?.length} {userAddedTokens.length === 1 ? t('Imported Token') : t('Imported Tokens')}
          </Text>
          {userAddedTokens.length > 0 && (
            <Button style={{ color: '#22CE77', background: 'transparent' }} onClick={handleRemoveAll}>
              {t('Clear all')}
            </Button>
          )}
        </Footer>
