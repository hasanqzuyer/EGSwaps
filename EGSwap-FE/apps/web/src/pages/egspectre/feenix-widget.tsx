...
import { feenixApiPost } from 'hooks/api'
import { EGSPECTRE_HOW_IT_WORKS_LINK } from 'config/constants'
import { FeenixButton, SupernovaButton, SwapButton } from './swap'
...
const FeenixWidget: React.FC<React.PropsWithChildren> = () => {
  ...
  useEffect(() => {
    const getTokenList = async () => {
      const networkRes = await feenixApiPost('quotes/networks')
      const tokenRes = await feenixApiPost('quotes/tokens?widget=feenix')
  ...
  const callPriceAPI = async () => {
    if (parseFloat(sendAmount) > 0) {
      ...
      try {
        ...
        const res = await feenixApiPost('quotes', params, 'POST', signal) // Pass the signal to the API call
  ...
  const checkMaintenanceStatus = async () => {
    const res = await feenixApiPost('admin/getMaintenanceMode')
  ...
  const handleExchange = async () => {
    ...
    try {
      ...
      const exchangeRes = await feenixApiPost('orders', params, 'POST')
      setIsLoading(false)
      clearInterval(intervalId)
      setLoadingDots(-1)
      if (exchangeRes.message === 'Success') {
        router.push(`/egspectre/${exchangeRes.data.egSpectreId}?widget=feenix`)
      }
    } catch (error) {
  ...
  const handleKeyPress = async (e: any) => {
    const orderResp = await feenixApiPost(`orders/${searchOrderId}`)

    if (e.key === 'Enter') {
      router.push(`/egspectre/${searchOrderId}?widget=feenix`)
    }
  }
  ...
  return (
    ...
    >
      <div
        className="flex flex-col p-[30px] border-transparent bg-gradient-to-r from-blue-500 to-purple-500 bg-clip-padding rounded-3xl max-w-[529px] w-full h-min z-10 relative"
        style={{
          background: 'linear-gradient(180deg, #262626 0%, #000000 50%, #262626 100%)',
        }}
      >
        <div className="flex justify-between items-center mb-3">
          <div className="flex flex-col">
            <div className="flex flex-row gap-2 items-center">
              <img src="/images/feenix.png" className="w-[50px] h-auto" />
              <span
                className="text-[28px] font-bold sm:text-3xl leading-[1.3]"
                style={{
                  backgroundImage: 'linear-gradient(180deg, #ffb902 0%, #b68400 100%)',
                  backgroundClip: 'text',
                  color: 'transparent',
            ...
            {/* <Flex alignItems={'center'} mt="15px">
              ...
              <Toggle
                ...
                scale="md"
                style={{
                  backgroundColor: isAnonymous ? '#E1E1E1' : '#ffb902',
                }}
                handleStyle={{ backgroundColor: '#ffb902 !important' }}
              />
          ...
          {isLoading && (
            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 200 200" className="w-[40px]">
              <linearGradient id="a7">
                <stop offset="0" stop-color="#FFB902" stop-opacity="0"></stop>
                <stop offset="1" stop-color="#FFB902"></stop>
              </linearGradient>
              <circle
                fill="none"
                stroke="url(#a7)"
        ...
        <div className="text-[0.875rem] font-medium text-[#feb902]">Fast, Secure, Zero Gas Fees</div>
        <div
        ...
        <div
          style={{ color: 'white' }}
          className="text-[12px] my-[15px] font-normal leading-normal text-[#dd801b] text-center"
        >
        ...
        <div className="flex flex-row items-center justify-center w-full mt-[15px]">
          ...
          >
            <input
              ...
              placeholder="Search by Feenix ID"
              value={searchOrderId}
        ...
        <div className="flex items-center justify-center my-5">
          <img src="/images/feenix-logo-bot.png" style={{ width: '30px', marginRight: '4px' }} />
          <span
            style={{
              marginRight: '4px',
              backgroundImage: 'linear-gradient(180deg, #ffb902 0%, #b68400 100%)',
...
FeenixWidget.displayName = 'egspectre'

export default FeenixWidget
