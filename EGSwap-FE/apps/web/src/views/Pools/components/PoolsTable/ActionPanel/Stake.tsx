...
const Staked: React.FunctionComponent<React.PropsWithChildren<StackedActionProps>> = ({ pool }) => {
  ...
  const stakedTokenDollarBalance = getBalanceNumber(stakedBalance.multipliedBy(egPrice), stakingToken.decimals)
  ...
  if (!account) {
    return (
      <ActionContainer>
        <ActionTitles>
          <Text fontSize="12px" bold color="#009BCC" as="span" textTransform="uppercase">
            {t('Start staking')}
          </Text>
        </ActionTitles>
        <ActionContent>
  ...
  if (notMeetRequired || notMeetThreshold) {
    return (
      <ActionContainer>
        <ActionTitles>
          <Text fontSize="12px" bold color="pool.textSubtle" as="span" textTransform="uppercase">
            {t('Enable pool')}
          </Text>
        </ActionTitles>
        <ActionContent>
      ...
      <ActionContainer>
        ...
        <ActionContent>
          <Button
            ...
            variant="secondary"
            style={{ background: 'linear-gradient(90deg, #21985D 0%, #009BCC 83.85%)', color: '#fff', border: 0 }}
          >
            {t('Enable')}
          </Button>
        </ActionContent>
